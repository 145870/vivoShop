...
<script>
	$(document).ready(function() {
		$.getJSON("RefreshDingDanServlet",function(data) {
			...
			$(".address_div:not(:lt(4)):not(:last)").addClass("hidden");

			// 为显示/隐藏按钮设置点击事件
			$("#showButton").click(function() {
				$(".address_div:not(:lt(4)):not(:last)").toggleClass("hidden"); // 切换其他地址框的显示/隐藏状态
			...
			// 给 div 元素添加点击事件
					...
					var name = $(this).find('.name').text(); // 直接使用$(this)获取当前点击的元素
					var tel = $(this).find('.tel').text(); // 直接使用$(this)获取当前点击的元素
					var sheng = $(this).find('.sheng').text(); // 直接使用$(this)获取当前点击的元素
					var shi = $(this).find('.shi').text(); // 直接使用$(this)获取当前点击的元素
					var xian = $(this).find('.xian').text(); // 直接使用$(this)获取当前点击的元素
					var xiang = $(this).find('.xiang').text(); // 直接使用$(this)获取当前点击的元素
					$("#name").text(name); // 将名字赋值给 #name 元素
					$("#tel").text(tel);
					$("#shengfen").text(sheng);
					$("#shiqu").text(shi);
					$("#xiancheng").text(xian);
					$("#xiangxi").text(xiang);
					// 移除其他 div 元素的样式
					$('.address_div').not(this).css("background-image","");
					// 添加当前点击的 div 元素的样式
					$(this).css("background-image","url('../image/图片边框.png')");
				...
				if ($(this).text() === "【默认地址】") {
					// 执行相关操作
					$.post("RefreshUpdateIdServlet",{id : address_id},function(data) {
					...
					// 获取复选框元素
					var checkbox = document.getElementById("mo");
					var moren = 1;
					// 添加事件监听器，当复选框状态改变时执行
					checkbox.addEventListener('change',function() {
					// 检查复选框是否被选中
					if (checkbox.checked) {
						onsole.log("复选框被选中，值为 0");
						moren = 0;
					} else {
						console.log("复选框未被选中，值为 1");
						moren = 1;
					}
				});
				$("#update").click(function() {
					...
					var xian = $('#xian').val();
					var xiang = $('#xiang').val();
					$.post("RefrechUpdateServlet",{sname : name,stel : tel,ssheng : sheng,sshi : shi,sxian : xian,sxiangxiadd : xiang,mo : moren,id : address_id},function(data) {
						// 在成功获取到数据后，将数据填充到对应的文本框中
						location.reload();
					});
				});
...
<body>
	<div id="box">
			...
			<div style="width: 1433px; display: flex; margin: 0 auto;">
				...
				<ul id="you">
					...
					<li><a
						...
						href="/vivoShop/PersonalCenter/shopping_cart.jsp">购物车</a></li>
					<li id="geren" style="position: relative;">个人中心
						<div id="deng_kuang">
							<c:choose>
								<c:when test="${user_profile == null}">
									<div id="gerenbox"
										style="display: flex; flex-direction: column;">
										<a href="/vivoShop/PersonalCenter/JSP/login.jsp"
											style="flex: 1; text-align: center; line-height: 35px">
											登录 </a> <a href="/vivoShop/PersonalCenter/JSP/register.jsp"
											style="flex: 1; text-align: center; line-height: 35px">
											注册 </a>
									</div>
								</c:when>
								<c:otherwise>
									<div id="gerenbox"
										style="display: flex; height: 105px; flex-direction: column;">
										<a href="/vivoShop/PersonalCenter/JSP/my_Mall.jsp"
											style="flex: 1; text-align: center; line-height: 35px">
											我的商城 </a> <a href="/vivoShop/PersonalCenter/JSP/my_order.jsp"
											style="flex: 1; text-align: center; line-height: 35px">
											我的订单 </a> <a
											href="/vivoShop/PersonalCenter/JSP/AccountCenter.jsp"
											style="flex: 1; text-align: center; line-height: 35px">
											账户中心 </a>
									</div>
								</c:otherwise>
					...
					<div class="navDiv">
						<div class="rebox">
							<div class="re" style='padding: 20px; padding-left: 80px'>
								<div class="tubox">
									<c:forEach items="${productsLists[typeLoop.index]}" var="pi"
										varStatus="loop">
										<c:if test="${loop.index < 6}">
											<a id="aaa"
												href="/vivoShop/front/jsp/DetailsServlet?id=${pi.information_id}">
												<div class='tu-div'>
		...
		<div id="body">
				...
				<div id="address_divid">
					...
					<div id="xinzengdiv">
						<div id="xinzengbox">
							...
							<p>新建收货地址</p>
							<div id="form_box">
								<form action="InsertAddServlet" method="post"
									onsubmit="return validateForm()">
									<div>
										<span>*</span>收货人: <input name="iname" class="input"
											style="margin-left: 27px;" />
									</div>
									<div>
										<span>*</span>手机号码： <input name="itel" pattern="[0-9]{11}"
											class="input" />
									</div>
									<div>
										<span>*</span>收货地址： <input type="text" placeholder="请选择"
											name="sheng" class="sinp" /> <input type="text"
											placeholder="请选择" name="shi" class="sinp" /> <input
											type="text" placeholder="请选择" name="xian" class="sinp" />
									</div>
									<div>
					...
					<div id="updatediv">
						<div id="updatebox">
							<span id="update_X" onclick="closeupdate()">X</span>
							<p>修改收货地址</p>
							<div id="form_boxup">
								<form action="" method="post">
									<div>
										<span>*</span>收货人: <input name="sname" class="input"
											id="iname" style="margin-left: 27px;" />
									</div>
									<div>
										<span>*</span>手机号码： <input name="stel" id="phone"
											pattern="[0-9]{11}" class="input" />
									</div>
									<div>
										<span>*</span>收货地址： <input type="text" placeholder="请选择"
											name="ssheng" class="sinp" id="sheng" /> <input type="text"
											placeholder="请选择" name="sshi" id="shi" class="sinp" /> <input
											type="text" placeholder="请选择" name="sxian" id="xian"
											class="sinp" />
									</div>
									<div>
										<span>*</span>详细地址： <input type="text" name="sxiangxiadd"
											class="input" id="xiang" />
									</div>
									<div>
										<input type="checkbox" class="default_input"
											name="sdefault_input" id="mo" /> 设为默认地址
									</div>
									<div>
						...
						<c:forEach items="${odList}" var="od">
							<tr class='tab-body'>
								<td>
									<div class="img_box">
										<img src="${od.imgURL }" />
									</div> <span class="guu">${od.pName} ${od.pavVals}</span>
								</td>
								<td class="jiage">${od.saleAmount}</td>
								<td class="shuliang">${od.quantity}</td>
								<td style="color: red;">${od.sumPrice}</td>
							</tr>
						</c:forEach>
			...
			<div id="xiang_box">
				<div id="yixuan_box">
					<div>已选${count}件商品，合计(不含运费) :</div>
					<div id="yixuan"></div>
				</div>
				<div id="yingfu_box">
					<label>应付总额:</label> <span id="price_sum">￥${sumPrices}</span>
				</div>
				<div id="shoujian_box">
					<div>收件人:</div>
					<div>
						<span id="name"></span> <span id="tel"></span>
					</div>
				</div>
				<div id="pei_box">
					<div>配送地址:</div>
					<div id="add_div">
						<span id="shengfen"></span> <span id="shiqu"></span> <span
							id="xiancheng"></span> <span id="xiangxi"></span>
					</div>
				</div>
				<div>
					<button id="btn_sumbit">
						<a onclick="goSettlementServlet()"
							style="text-decoration: none; color: white;">提交订单</a>
					</button>
				</div>
			</div>
		</div>
	...
	<script>
		...
		//提交订单
		function goSettlementServlet() {
			//获取收件人姓名
			var name = $("#name").text();
			//获取收件人电话
			var tel = $("#tel").text();
			//获取地址
			var sheng = $("#shengfen").text();
			var shi = $("#shiqu").text();
			var xian = $("#xiancheng").text();
			var xiang = $("#xiangxi").text();

			//使用ajax提交数据新增订单

			$.ajax({
				url : 'SettInsertServlet', // 
				type : 'POST', // 请求类型
				data : {
					name : name,
					tel : tel,
				...
				success: function(response) {
				    ...
				    if (jsonResponse.resu == true) {
				        window.location.href = '/vivoShop/front/jsp/SettlementServlet';
				    } else {
				        alert('请求失败，请重试！');
				    }
				}
