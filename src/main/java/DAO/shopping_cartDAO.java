...
public class shopping_cartDAO extends BaseDAO{
	...
	//查询购物车的数据
	public List<shopping_cartDetails> quershopping_carts_data(int uid){
		String sql = "SELECT usc.`id`,usc.`user_id`,p.`information_name`,pav.`attr_vals`,pav.`sale_amount`,usc.`quantity`,(pav.`sale_amount`*usc.`quantity`) as 'sumPrice',piu.`url`,p.id\r\n"
				...
				+ " WHERE usc.`user_id` = ?  AND piu.`url` LIKE '%DM_%_001.%'";
	...
	public int deleteShoppingcart(long id) {
		String sql = "DELETE FROM user_shopping_cart WHERE id = ?";
